1.脚本功能
  生成创建txt外表、Orc表、ES表、Hyperdrive表、Holodesk表、由外表插入orc表的insert语句。
...
3.替换orc ddl中的分桶字段和分桶数目
  3.1 填写replaceBucketColumn/buckets.conf
  3.2 修改replaceBucketColumn/replace.sh
      将OWNER的值改为要替换的用户名
  3.3 运行replaceBucketColumn/replace.sh